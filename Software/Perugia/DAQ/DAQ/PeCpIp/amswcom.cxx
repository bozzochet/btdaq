...
#include <errno.h>
//#include <asm/bitops.h> // nedded to comment this line to compile on fedora10
...
int AMSWcom::SendCommandPCI(int link, const u_char command[], int csize, u_short wordarray[], int *datasize, int mode24){
	...
	*datasize=0;
	...
	do {
		...
		*datasize+=fragsize;
		//printf("return value is %d (amw_rx_packet) | fragsize = %d | link = %d\n", ret, fragsize, link);
		if ( ret < 0 ) { 
			return -4;  //RX failed ...
		}
...
void AMSWcom::LecroyRead(unsigned int addrl, QList *qlist) {
	...
	if (DEBUG)  { for (int i=0; i<cnt; i++) printf("%04x ", command[i]); printf("\n"); }
...
unsigned short AMSWcom::WriteJLV1Conf(unsigned int selfaddr){
	...
	//   if (out==0) { printf("Sorry, file problem... with %s \n I give up! \n",filename); return 1; }
	//   while(!feof(out)){
	//     fscanf(out,"%04hx",&a);
	//     array[cnt]=a;
	//     cnt++;
