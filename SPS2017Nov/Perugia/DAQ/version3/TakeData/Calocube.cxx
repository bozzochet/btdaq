...
int Calocube::open_file (char *run_number_name, bool ped_dat){
	file_name = this->compose_name(run_number_name, ped_dat).c_str();
...
int Calocube::open_file (int run_number, bool ped_dat){
	file_name = this->compose_name(run_number, ped_dat).c_str();
...
string Calocube::compose_name (char *run_number_name, bool ped_dat){
	string name;
	char buffer[512];
	if(ped_dat)
		sprintf(buffer, "%s/CC%s.ped", data_path, run_number_name);
	else
...
string Calocube::compose_name (int run_number, bool ped_dat){
	string name;
	char buffer[512];
	if(ped_dat)
		sprintf(buffer, "%s/CC%d.ped", data_path, run_number);
	else
