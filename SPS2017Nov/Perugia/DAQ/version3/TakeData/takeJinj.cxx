...
struct header {//for file writing NOT in AMSBlock
	...
	unsigned int refmask[NSLAVE];// JinjSlave refmask
};
...
int Calibrate(AMSWcom* node) {
	...
	int init_calo = calocube->init();
	if(init_calo==-1 || init_calo==-3) {
		printf("Error during calocube initialization : Force Exit\n");
		return 1;
		if(calocube->close()!=0) printf("Error during calocube closing\n");
		delete calocube;
		calocube =NULL;
	} else if(init_calo==-2 && !calocube->is_enabled()) {
		delete calocube;
		calocube =NULL;
	} else if(daq!=1 || WritingMode!=1) {
		printf("Common data taking with Calocube requires both daq=1 and WritingMode=0 : Force Exit");
		return 1;
		if(calocube->close()!=0) printf("Error during calocube closing\n");
		delete calocube;
		calocube =NULL;
...
#ifdef CALIBRATION
	if(calocube) {
		time_t calnum = time(NULL);
		struct tm *calnumstruct = localtime( & calnum );

		char calnumname[256];
		strftime(calnumname,256,"%Y%m%d_%H%M%S", calnumstruct);

		printf("\n=== Calocube Run Start ===============================================\n");
		if(calocube->open_file(calnumname, true)!=0) return 1;
		printf("Opening Calocube File %s\n", (calocube->get_file_name()).c_str());

		printf("Configuring Calocube Acquisition\n");
		if(calocube->conf_roc()!=0) return 1;
		sleep(1);

		printf("Starting Calocube Acquisition\n");
		if(calocube->enable_trigger()!=0) return 1;
		printf("====================================================================\n");
	}
#endif
	//----------------------------------
...
#ifdef CALIBRATION
	if(calocube)
		for(int ievent=0; ievent<0xfc; ++ievent)
			if(calocube->take_data()!=0) {
				if(calocube->enable_trigger()!=0)
					return 1;
			}
#endif
...
int StartRun(AMSWcom *node, int nevents, int fake) {
	...
	char runnumname[256];
	//sprintf(runnumname, "%d", (int)runnum);
	strftime(runnumname,256,"%Y%m%d_%H%M%S", runnumstruct);
	...
	if (ancillary_code<0) {
		...
		if(calocube) {
			...
			sprintf(newfilename,"%s/%s.dat", JJ->CPars->DATAPATH, runnumname);

			char systemcommand[256];
			sprintf(systemcommand, "ln -s %d.dat %s", runnum, newfilename);
			printf("%s\n", systemcommand);
	...
	if(calocube) {
		printf("\n=== Calocube Run Start ===============================================\n");
		if(calocube->open_file(runnum, false)!=0) return 1;
		printf("Opening Calocube File %s\n", (calocube->get_file_name()).c_str());
		...
		char systemcommand[256];
		sprintf(systemcommand, "ln -s CC%d.dat %s", runnum, (calocube->compose_name(runnumname, false)).c_str());
		printf("%s\n", systemcommand);
		system(systemcommand);
	}
	...
	if(calocube) {
		printf("\n=== Calocube Perform Initial Trigger Check ==========================\n");
		printf("Trigger was enabled both on Calocube and AMS\n");
		//At this point we expect to have a trigger in Calocube and no trigger in AMS if we have the following trigger configuration
		//- concidence physics trigger
		//- 1 Hz pedestal trigger

		//The following block:
		//- check that there is a trigger in Calocube but not in AMS
		//- enable calocube trigger, bringing Calocube BUSY to 0 and therefore removing the VETO on successive triggers
		...
		while(!ready) {
			...
			if(elapsed>calocube->init_time_out) {
				printf("Initial Trigger Check: Calocube has not received any trigger : Force Exit\n");
				return 1;
			}

			if(calocube->check_data_ready(ready)!=0)
				return 1;

			if(ready) {
				if((calocube->read_data(false)!=0))//read event and throw it away
					return 1;

				usize=0;
				usize=node->GetEvent(0xffff);
				if (usize>0) {
					printf("Initial Trigger Check: AMS has received a trigger as well : Force Exit\n");
					return 1;
				}

				//This is necessary to manually enable AMS trigger
				//Question how much time after TriggerOn AMS trigger is really On?
				printf("Please press ENTER to start data taking (if trigger have been already turned ON)\n");
				while( getchar() != '\n' );

				if((calocube->enable_trigger()!=0))//enable calocube trigger
					return 1;
			}
		}
		printf("=====================================================================\n");
	...
	while(ControlOn) {
		...
		if(is_calocube_enabled) {
			...
			if(elapsed>calocube->loop_time_out) {
				printf("\n=====================================================================\n");
				printf("Loop over event: Time out of %.0f seconds reached : Print Status and Force Exit\n", calocube->loop_time_out);
				bool calocube_ready, ams_ready;
				++errcnt;

				if(calocube->check_data_ready(calocube_ready)!=0) { //eugenio : mi sembra meglio break di ControlOn=0, dato che evita tutte le chiamate successive...
					++errcnt;
				...
				if(is_calocube_enabled) {
					usize=0;
					usize=node->GetEvent(0xffff);
					ams_ready = (usize>0)? true : false;
					printf("Status: (Calocube, AMS) trigger condition is (%d, %d)\n", (calocube_ready? 1 : 0), (ams_ready? 1 : 0));//ams_ready);
					++errcnt;
					is_calocube_enabled=false;
				}
			}
		...
		//eugenio moved the following lines inside next "if" condition
		//		Head.DSPSIZE=usize*2+4;
		//		Head.SIZE=(ushort)(8+2+Head.JMDCSIZE+2+Head.DSPSIZE);
		...
		if (usize>0) {
			...
			if(daq){
				if (WritingMode==0){
					...
					//Write AMS event
					fwrite(&header_size, sizeof(header_size), 1, datafile);
					...
					if(is_calocube_enabled) {
						...
						for(int i=0; i<10; ++i) {
							bool roc_ready;
							if(calocube->check_data_ready(roc_ready)!=0)
								is_calocube_enabled=false;
							usize=0;
							usize=node->GetEvent(0xffff);
							bool ams_ready = ((usize>0) ? true : false); 
							if(ams_ready)
								cout << i << " " << roc_ready << " " << ams_ready << " : " << usize << " - " << inizio << endl;
							usleep(1000);
						}*/

						fwrite(&header_size, sizeof(header_size), 1, datafile);
						fwrite(&Head, header_size, 1, datafile);
					}
					...
					if(is_calocube_enabled) {
						//fwrite(&DSPCRC16, sizeof(DSPCRC16), 1, datafile); //eugenio removed this word element from both calocube and AMS
						usize=0;
						usize=node->GetEvent(0xffff);
						if (usize>0) {
							printf("Loop : AMS has received more than one trigger event : Force Exit\n");
							++errcnt;
							is_calocube_enabled=false;
						}
