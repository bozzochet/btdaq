...
char *title = {"         PDS-5  Controller"};         // global variable
  ...
  for (i=0; i<n; i++) {
    ...
    for (ii=0; ii<4; ii++) {
      ...
      if (go_on(err)) {
        ...
        exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+adc, &dig, &ana, &err);
        if (!err) {
  ...
  for (i=0; i<n; i++) {
    ...
    for (ii=0; ii<8; ii++) {
      ...
      if (go_on(err)) {
        ...
        pack16(&cmd, 0x01E0, adr);
        pack16(&cmd, 0x001F, cha);
        exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+0, &dig, NULL, &err);
        error |= err;
