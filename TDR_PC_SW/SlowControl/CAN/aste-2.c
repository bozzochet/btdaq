...
// (this program generates PPS by GPSE board)
//
// A.Lebedev May-2006...
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    while (!timer(1, EXPIRED, 1.0));
    Write_serial_port(0, 1, (int8*)"a", &err);
    if (err) {
      printf("(1) Write_serial_port failed (err=%04X)\n", err);
      exit(1);
    }

    Read_serial_port(0, 8192, &n_rcv, d_rcv, &err);
    if (err) {
      printf("(1) Read_serial_port failed (err=%04X)\n", err);
      exit(1);
    }
    ...
    write_LVDS_bus(base+3, SEND_PPS, &err);
    if (err) {
      printf("SEND_PPS via USCM %c (%03X) failed (err=%04X)\n",
              uscm ? 'B' : 'A', USCM_ID, err);
      exit(1);
    }
    timer(1, START);
    ...
    Write_serial_port(0, 1, (int8*)"a", &err);
    if (err) {
      printf("(2) Write_serial_port failed (err=%04X)\n", err);
      exit(1);
    }

    Read_serial_port(0, 8192, &n_rcv, d_rcv, &err);
    if (err) {
      printf("(2) Read_serial_port failed (err=%04X)\n", err);
      exit(1);
    }
    printf("n_rcv = %d, d_rcv = ", n_rcv);
    for (i=0; i<n_rcv; i++) printf("%c", d_rcv[i]);
