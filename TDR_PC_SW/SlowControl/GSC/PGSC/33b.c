...
char *host[5] = { NULL, NULL, NULL, "acarbon",  "pcX.hrdl" };
char *user[5] = { NULL, NULL, NULL, "lebedev",  "hropr"    };
char *pass[5] = { NULL, NULL, NULL, "KILLCOMM", "hi!rate?" };
char *path[5] = { "/dath1/HRDL/EVENTS/",
                  "/dath3/HRDL/EVENTS/",
                  "/stage/HRDL/EVENTS/",
                  "/dath1/HRDL/EVENTS/",
                  "/home/hropr/data/LINKS/"};
...
int open_AMS_event_file( void) {
  ...
  sprintf( event_path, "%s%6.6d", path[event_node_index], event_file_number);

  if (host[event_node_index] != NULL) {
    if (Ftp_Connect( host[event_node_index], 
                     user[event_node_index], 
                     pass[event_node_index]) != 1) {
      if (TRACE_LEVEL > 0) {
        printf( "failed to connect to host %s\n", host[event_node_index]);
      }
      return FAIL;
    }
    else {
...
//==============>>>>>>>>>>>>>>
  ...
  for (; event_node_index<3; event_node_index++) {
    for (;
         event_file_number <= last_file_number_in( path[event_node_index]);
         event_file_number++) {
...
int data_inventory( int mode) {
  int i, min, sec;
  char c;
  static integer32 run_ID_table[3000];
...
int new_data_inventory( int mode) {
  ...
  static total[2], normal[2], laser[2], calib[2], bad[2];
  ...
  if (mode) {
    ...
    for (i=0; i<3000; i++) {
      ...
      int ijk;
      if (run[i].ID == 0) break;
      if (run[i].ID <= 0x3591A11A) ijk = 0;
      else                         ijk = 1;
      total_nb_of_events += run[i].nb_of_events;
      total[ijk] += run[i].nb_of_events;
      if (run[i].type == 0x07) {
        total_nb_of_laser_events += run[i].nb_of_events;
        laser[ijk] += run[i].nb_of_events;
      }
      if (run[i].type == 0x1F) {
...
///*
    ...
    printf( "Total time = %d mins\n", ((int) total_time + 30) / 60);
/*
    printf( "LV3 rejection:: Total = %d = normal:%d + laser:%d + calib:%d + bad:%d\n",
        total[0], normal[0], laser[0], calib[0], bad[0]);
    printf( "Without LV3:: Total = %d = normal:%d + laser:%d + calib:%d + bad:%d\n",
        total[1], normal[1], laser[1], calib[1], bad[1]);
*/
    if ((database = fopen( "database", "wb")) != NULL) {
      fwrite( run, sizeof( run), 1, database);
      fclose( database);
