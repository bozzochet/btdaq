...
//-------------------------------------------------
		if (argv<2) {
			...
			for (int ii=0;ii<NJINF;ii++) {
				RefMask(CPars,atoi(argc[1]), ii, "");
				ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), ii, "pdf", "");
			}
		}
		if (argv>2) {
			char type[255];
			sprintf(type,"pdf");
			if (!strcmp(argc[2],"ps")) sprintf(type,"ps");
			if (argv==3) {
				for (int ii=0;ii<NJINF;ii++) {
					RefMask(CPars,atoi(argc[1]), ii, "");
					ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), ii, type, "");
				}
			}
			else if (argv=4) {
				for (int ii=0;ii<NJINF;ii++) {
					RefMask(CPars,atoi(argc[1]), ii, argc[4]);
					ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), ii, type, argc[3]);
				}
			}
			else if (argv>4) {
				sprintf(CPars->CALPATH,"%s/",argc[4]);
				for (int ii=0;ii<NJINF;ii++) {
					RefMask(CPars,atoi(argc[1]), ii, argc[3]);
					ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), ii, type, argc[3]);
				}
			}
		}
...
int SummaryComplete(char *dir, int run_number, int jinfnum, char *outkind, char *nameprefix) {
  ...
  char nameouttemp[255];
	...
	sprintf(systemcommand,"gv %s &",nameout);
