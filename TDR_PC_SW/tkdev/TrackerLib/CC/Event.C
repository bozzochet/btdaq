...
void Event::Info(){
  ...
  for (int ii=0;ii<GetnTrTracks();ii++) {
    strstream ost(msg, 1024);
    GetTrTrack(ii)->_printEl(ost);
    sout.append(msg);
  }
...
void Event::GetNRawClusterPlaneSide(int num[8][2]){
  for(int ii=0;ii<(int)fTrRawClusters.size();ii++){
...
void Event::GetNTrClusterPlaneSide(int num[8][2]){
  for(int ii=0;ii<(int)fTrClusters.size();ii++){
...
void Event::GetNRecHitPlane(int num[8][2]){
  for(int ii=0;ii<(int)fTrRecHits.size();ii++){
