...
void SlowGui::TrackerMonitor() {
  ...
  fTrackerMonitorMainFrame->SetWindowName(Form("Tracker Monitor tcrate %d",TCrate));
  ...
  gClient->GetColorByName("#75D36D",PwgColDark[0]); // green
  gClient->GetColorByName("#75A9E0",PwgColDark[1]); // blue
  gClient->GetColorByName("#DE9D74",PwgColDark[2]); // red
  gClient->GetColorByName("#DDD873",PwgColDark[3]); // yellow
  ...
  for (int pwg=0; pwg<4; pwg++) {
	...
	int lad=InnerTracker.FindLadderIndex(TCrate,pwg,pwgpos);
	//printf("pwg=%d, pwgpos=%d, lad=%d\n", pwg, pwgpos, lad);
	TkLadder *ladder=InnerTracker.GetLadder(lad);
	
	// "L 09 GI 010" group frame
	fGroupFrame[pwg][pwgpos] = new TGGroupFrame(hframe,(lad==MAXLAD)?"":Form("%s - TDR %d",ladder->GetName(), ladder->GetTdr()),kVerticalFrame ,uGC->GetGC(),ufont->GetFontStruct(),PwgCol[pwg]);
	...
	fGroupFrame[pwg][pwgpos]->SetTitlePos(TGGroupFrame::kCenter);
	// for tests
	//fGroupFrame[pwg][pwgpos]->SetTextColor((pwgpos!=3)?0:0x00FFFF); // magenta ff00ff
	//fGroupFrame[i]->SetLayoutBroken(kTRUE);
	
	TGHorizontalFrame *myhframe=new TGHorizontalFrame(fGroupFrame[pwg][pwgpos]);
	...
	if (lad!=MAXLAD) {
	  TRootEmbeddedCanvas *fRootEmbeddedCanvas = new TRootEmbeddedCanvas(Form("ce%03d",lad),myhframe,70,60); // was 50,40
	...
	fGroupFrame[pwg][pwgpos]->AddFrame(myhframe, new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
	 
	//fGroupFrame[i]->SetLayoutManager(new TGVerticalLayout(fGroupFrame[i]));
	 
	fHProgressLeak[pwg][pwgpos]=new TGHProgressBar(fGroupFrame[pwg][pwgpos], 140, TGProgressBar::kProgressBarStandardWidth, 0xc0c0c0, TGFrame::GetDefaultSelectedBackground(), uGC2->GetGC(), progressfont->GetFontStruct(),kDoubleBorder|kSunkenFrame);
	...
	fGroupFrame[pwg][pwgpos]->AddFrame(fHProgressLeak[pwg][pwgpos], new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
	 
	hframe->AddFrame(fGroupFrame[pwg][pwgpos], new TGLayoutHints(kLHintsLeft | kLHintsTop,2,2,2,2));
    ...
    fStatusPwg[pwg]->SetText(Form("Power group %d",pwg),0);
    ...
    //    (fStatusPwg[pwg]->GetBarPart(0))->SetForegroundColor(0xff0000);
    testpb[pwg]=new TGHProgressBar(fStatusPwg[pwg]->GetBarPart(1), 100, (TGProgressBar::kProgressBarStandardWidth), 0xc0c0c0, TGFrame::GetDefaultSelectedBackground(), uGC2->GetGC(), progressfont->GetFontStruct(),kDoubleBorder|kSunkenFrame);
    testpb[pwg]->SetBarType(TGProgressBar::kStandard);
    testpb[pwg]->SetFillType(TGProgressBar::kSolidFill);
    testpb[pwg]->ShowPosition(kTRUE,kFALSE,"%.2f");
    testpb[pwg]->SetMin(0);
    testpb[pwg]->SetMax(90);
    testpb[pwg]->SetBarColor("red");
    testpb[pwg]->SetForegroundColor(0xffff00);
    testpb[pwg]->SetBackgroundColor(0x010b6c);
    testpb[pwg]->Format("%2.1f V");
    (fStatusPwg[pwg]->GetBarPart(1))->AddFrame(testpb[pwg], new TGLayoutHints(kLHintsRight | kLHintsTop,2,2,2,2));
  ...
  //fTrackerMonitorMainFrame->Resize(800,900);
  fTrackerMonitorMainFrame->MapWindow();

  SetPosition(TCrate,1);
...
void SlowGui::UpdateTKTrips(int pwg, int pwgpos, int typ, int cnt) {
  ...
  TGGroupFrame *groupframe = fGroupFrame[pwg][pwgpos];

  if (groupframe==0) return;
  ...
  //  groupframe->SetBackgroundColor((cnt)?PwgColDark[pwg]:PwgCol[pwg]);
  groupframe->SetTextColor((cnt)?0xFF00FF:0);

  gClient->NeedRedraw(groupframe);
...
void SlowGui::DisplayTKVoltages(int pwg, double voltage) {
  ...
  testpb[pwg]->SetPosition(voltage);

  if ((voltage>61 && voltage<79) || (voltage<59) || (voltage>81) ) testpb[pwg]->SetBarColor(0xFF0000); // red
  else if (voltage<=61 && voltage>=59) testpb[pwg]->SetBarColor(0x00FFFF); // cyan
  else testpb[pwg]->SetBarColor(0x59D454); // green
...
void SlowGui::FillTKGraph(int pwg, int pwgpos, double val) {
  ...
  //  if (npoints==0) {
  TkLadder *ladder=InnerTracker.FindLadder(TCrate,pwg,pwgpos);
  val0=ladder->GetLc80();
    //  } 
  //else {
  ...
  if (npoints>0) {
    ...
    TCanvas *c=(TCanvas*)(gROOT->FindObject(Form("ccc%d",InnerTracker.FindLadderIndex(TCrate,pwg,pwgpos))));  
    if (c) {
      //gROOT->SetStyle("StyleWhite");
      LadCanZoom[pwg][pwgpos]->cd();
      LadGraph[pwg][pwgpos]->Draw("current:time","","l");
...
void SlowGui::UpdateCurrent2(int pwg, int pwgpos, double current) {
  ...
  //if (lc80<0.1) lc80=0.1;
  //int val=100*(log10(lc80)+1); // max foreseen 200 ua: (log10(200)+1)*100 = 330

  //  fHProgressLeak[pwg][pwgpos]->SetBarColor((current>1.5*lc80 || current>5+lc80)?0xff0000:0x59d454);
  //fHProgressLeak[pwg][pwgpos]->SetBarColor((current>1.5*lc80)?0xff0000:0x59d454);
  fHProgressLeak[pwg][pwgpos]->SetBarColor((current>1.5*lc80 && current>5+lc80)?0xff0000:0x59d454);
