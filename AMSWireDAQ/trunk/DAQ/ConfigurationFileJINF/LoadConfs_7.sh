...
OUTPUT=`./TESTjmdc 0 173f 45 | grep rxdone`
usleep 100000
echo Write Flash on node 0 173f: $OUTPUT
if [ "$OUTPUT" != "rxdone = 0000" ]
 then
  let "CMDERRORS=$CMDERRORS+1"
fi
...
rm -f ./file.dat
cp -fv ./dotdats/configfile_7.dat ./file.dat
OUTPUT=`./TESTjmdc 2 173f 45 | grep rxdone`
usleep 100000
echo Write Flash on node 2 173f: $OUTPUT
if [ "$OUTPUT" != "rxdone = 0000" ]
 then
  let "CMDERRORS=$CMDERRORS+1"
fi
