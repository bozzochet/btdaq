...
int TrigClass::Calib2PassTriggerOn(int dtime){
  LPRINTF("Calib2PassTriggerOn...\n");
  int ret=0;
  if(!_PJLV1) {
    PRINTF("For now Second Pass Calibration is the same of Normal Calibration!\n");
    if (!fake){
      ret=Write(0,0);//switching off data trigger for precaution
      ret+=Write(1,1);
      if (ret<0) return ret;
  ...
  else ret=_PJLV1-> Calib2PassTriggerOn(dtime);
  return ret;
}
