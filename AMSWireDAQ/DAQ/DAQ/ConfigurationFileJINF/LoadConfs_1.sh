...
WRTERRORS=0
ERSERRORS=0
OUTPUT=`./TESTjmdc 0 093f 7 | grep 7001`
if [ -n "$OUTPUT" ]
 then
  OUTPUT=`./TESTjmdc 0 093f 47 7001 | grep rxdone`
  usleep 200000
  ...
  if [ -n "$OUTPUT" ]
    then
    echo DSP Code 7001 Not Erased on node 0 093f!!!
    let "ERSERRORS=$ERSERRORS+1"
  fi
 else
  echo DSP Code 7001 not found on node 0 093f, so no erasing is needed
fi
rm -f ./file.dat
...
if [ -z "$OUTPUT" ]
 then
  echo DSP Code 7001 Not Loaded on node 0 093f!!!
  let "WRTERRORS=$WRTERRORS+1"
fi
OUTPUT=`./TESTjmdc 2 093f 7 | grep 7001`
if [ -n "$OUTPUT" ]
 then
  OUTPUT=`./TESTjmdc 2 093f 47 7001 | grep rxdone`
  usleep 200000
  ...
  if [ -n "$OUTPUT" ]
    then
    echo DSP Code 7001 Not Erased on node 2 093f!!!
    let "ERSERRORS=$ERSERRORS+1"
  fi
 else
  echo DSP Code 7001 not found on node 2 093f, so no erasing is needed
fi
rm -f ./file.dat
cp -fv ./dotdats/configfile_1.dat ./file.dat
...
if [ -z "$OUTPUT" ]
 then
  echo DSP Code 7001 Not Loaded on node 2 093f!!!
  let "WRTERRORS=$WRTERRORS+1"
fi
