...
WRTERRORS=0
ERSERRORS=0
OUTPUT=`./TESTjmdc 0 033f 7 | grep 7021`
if [ -n "$OUTPUT" ]
 then
  OUTPUT=`./TESTjmdc 0 033f 47 7021 | grep rxdone`
  usleep 200000
  ...
  if [ -n "$OUTPUT" ]
    then
    echo DSP Code 7021 Not Erased on node 0 033f!!!
    let "ERSERRORS=$ERSERRORS+1"
  fi
 else
  echo DSP Code 7021 not found on node 0 033f, so no erasing is needed
fi
rm -f ./file.dat
...
if [ -z "$OUTPUT" ]
 then
  echo DSP Code 7021 Not Loaded on node 0 033f!!!
  let "WRTERRORS=$WRTERRORS+1"
fi
OUTPUT=`./TESTjmdc 2 033f 7 | grep 7021`
if [ -n "$OUTPUT" ]
 then
  OUTPUT=`./TESTjmdc 2 033f 47 7021 | grep rxdone`
  usleep 200000
  ...
  if [ -n "$OUTPUT" ]
    then
    echo DSP Code 7021 Not Erased on node 2 033f!!!
    let "ERSERRORS=$ERSERRORS+1"
  fi
 else
  echo DSP Code 7021 not found on node 2 033f, so no erasing is needed
fi
rm -f ./file.dat
cp -fv ./dotdats/configfile_0.dat ./file.dat
...
if [ -z "$OUTPUT" ]
 then
  echo DSP Code 7021 Not Loaded on node 2 033f!!!
  let "WRTERRORS=$WRTERRORS+1"
fi
