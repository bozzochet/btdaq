...
char* Jinf::PrintAllEventNumber(int log,int Jinfnum) {
  ...
  sprintf(numbers,"Jinf %02d: %6d",Jinfnum,jinfevents);
  sprintf(numbers,"%s\nTdrs:",numbers);
  ...
  for (int ii=0;ii<NTDRS;ii++)
    if(CPars->refmask&(1<<ii) && CPars->mode[ii]) {
      ...
      sprintf(numbers,"%s%2d: %6d | ",numbers,ii/*+Jinfnum*100*/,tdrevents[ii]);
    }
...
int Jinf::WriteCalibPar(AMSWcom* node, int tdrnum){
  ...
  unsigned short addr=0x1F25;
	...
	unsigned short dmCALIBR_DT_DELAY = 0xEA6;   // dmCALIBR_DT_DELAY default: EA6
	...
	dmCALIBR_DT_DELAY = CPars->DeltaTdoubTrig[tdrnum];
	...
	node->Event[9]=0xffff & dmCALIBR_DT_DELAY;

  node->WriteMemory(amswaddr,addr,0x10,kDSP_DM);
