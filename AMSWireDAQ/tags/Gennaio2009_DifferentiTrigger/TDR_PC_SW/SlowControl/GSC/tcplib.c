...
//~--- send the request packet ---
  ...
  if ( FD_ISSET(s, &rd)) {
    pReply->Error = 0x1203;
    printf("\n*** select: Ready to read(?), err = 0x%04X\n", pReply->Error);
    return;
  }
  if (!FD_ISSET(s, &wr)) {
    pReply->Error = 0x1204;
    printf("\n*** select: Not ready to write, err = 0x%04X\n", pReply->Error);
    return;
  }
  if ( FD_ISSET(s, &er)) {
    pReply->Error = 0x1205;
    printf("\n*** select: Exception happened, err = 0x%04X\n", pReply->Error);
    return;
  }
...
//~--- receive the reply packet ---
  ...
  while (1) {
    ...
    while (n_rcv < Reply_Packet_Length) {
      ...
      if (!FD_ISSET(s, &rd)) {
        pReply->Error = 0x120D;
        printf("\n*** select: Not ready to read, err = 0x%04X\n", pReply->Error);
        return;
      }
      if ( FD_ISSET(s, &er)) {
        pReply->Error = 0x120E;
      ...
      if (r ==  0) {
        pReply->Error = 0x120F;
        printf("\n*** read: Server closed connection, err = 0x%04X\n", pReply->Error);
        return;
      }
      if (r == -1) {
        pReply->Error = 0x1210;
