// file PDS-5-OLD.c
//
// PDS-5-OLD Controller - uses old CGS protocol
//
// A.Lebedev Dec-2007...
...
char *title = {"PDS-5-OLD  Controller"};              // global variable
  ...
  for (i=0; i<n; i++) {
    ...
    for (ii=0; ii<4; ii++) {
      ...
      if (go_on(err)) {
        ...
        exec_PDS_cmd(cmd, adc, &dig, &ana, &err);
        if (!err) {
  ...
  for (i=0; i<n; i++) {
    ...
    for (ii=0; ii<8; ii++) {
      ...
      if (go_on(err)) {
        ...
        pack16(&cmd, 0x01E0, adr);
        pack16(&cmd, 0x001F, cha);
        exec_PDS_cmd(cmd, 0, NULL, NULL, &err);
        error |= err;
      }
      if (go_on(err)) {
        cha = brd[i].t_r;
        ...
        pack16(&cmd, 0x01E0, adr);
        pack16(&cmd, 0x001F, cha);
        exec_PDS_cmd(cmd, 0, &dig, NULL, &err);
        error |= err;
