...
char *title = {"LDDR - 2  Controller"};               // global variable
...
void set_DAC_callback(FL_OBJECT *obj, long data);
...
//AFTER_RUN_FLASH_CALL       = refresh_objects;
  ...
  if (_H_) {
    ...
    create_LCTL_power_panel(     375, 470);
    create_USCM_control_H2_panel(660, 475);
  }
  else {
...
void create_LDDR_board_panel(int x0, int y0, int ind) {
  ...
  char b[80];
  int x, y, w, h;
  _b dac_msk[1] = {{0, 0x00000FFF, 4096}};
  _b adc_msk[1] = {{0, 0xFFFFFFFF, -1}};
  
//~----- "horizontal" layout ------

  if (_H_) {
    int w1 = 70;
    int w2 = 50, w3 = 50, w4 = 40, w5 = 40, w6 = 45;
    int w7 = 60, w8 = 60, w9 = 50;
    int w10 = 30, w11 = 50;
...
//~--
    ...
    w = 5 + w1 + w2 + w3 + 
        2 * (w4 + w5 + w6 + w7 + w8 + w9) + 5 + w10 + w11 + 5;
    h = 5 + h1 + h2 + 2 * h3 + 5;
    fl_add_box(FL_DOWN_BOX, x0, y0, w, h, " ");
...
//~--
    ...
    for (j=0; j<2; j++) {
      ...
      y = y + h2;
      create_btn_hstring(
        1, x, y, w7, h3, NULL, 'M', &w_dac[ind][j], dac_msk, &DAC_BTN[ind][j]);
      obj = fl_add_button(FL_HIDDEN_BUTTON,        x, y, w7/3, h1, "100");
        fl_set_object_callback(obj, set_DAC_callback, 2*ind+j);
      obj = fl_add_button(FL_HIDDEN_BUTTON,   x+w7/3, y, w7/3, h1, "10");
        fl_set_object_callback(obj, set_DAC_callback, 2*ind+j);
      obj = fl_add_button(FL_HIDDEN_BUTTON, x+2*w7/3, y, w7/3, h1, "1");
        fl_set_object_callback(obj, set_DAC_callback, 2*ind+j);
...
//~--
    ...
    for (j=0; j<2; j++) {
      ...
      x = x + w5;
      create_btn_hstring(
        1, x, y, w2, h1, NULL, 'M', &w_dac[ind][j], dac_msk, &DAC_BTN[ind][j]);
      obj = fl_add_button(FL_HIDDEN_BUTTON, x, y, w2/3, h1, "100");
        fl_set_object_callback(obj, set_DAC_callback, 2*ind+j);
      x = x + w2 / 3;
      obj = fl_add_button(FL_HIDDEN_BUTTON, x, y, w2/3, h1, "10");
        fl_set_object_callback(obj, set_DAC_callback, 2*ind+j);
      x = x + w2 / 3;
...
void set_DAC_callback(FL_OBJECT *obj, long data) {
  ...
  pack32(&w->inp[0], 0x0FFF, MOD(w->inp[0]+d, 4096));
  w->sta = status_NA;
...
//~--
  ...
  if (!err) {
    ...
    for (j=0; j<2; j++) {
      ...
      for (k=0; k<16; k++) if (k == (2 * ind + j)) dat[k] = w->inp[0] & 0x0FFF;
    }
    write_DAC(dat, &err);
    for (j=0; j<2; j++) {
      _ww *w = &w_dac[ind][j];
...
void update_LDDR_board_panel(int ind) {
  ...
  for (j=0; j<2; j++) {
    ...
    sprintf(b, "%5.2f", 0.050 * (float)w_dac[ind][j].inp[0]);
    show_btn_string(1, b, &DAC_BTN[ind][j]);

    sprintf(b, "%5.2f", 0.050 * (float)r_dac[ind][j].rea[0]);
    show_dat_box_string(1, NULL, b, &DAC_BOX[ind][j]);

    sprintf(b, "%5.2f", 0.110 * (float)r_adc[ind][j].rea[0]);
