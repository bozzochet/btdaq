...
char *title = {"         PDS-4  Controller"};         // global variable
...
//~-- PDS Protocol Selector

SW_BTN *PROTOCOL_BTN;
_ww w_protocol;
_b protocol = {0, 0x00000001, 2};
char *protocols[2] = {"OLD", "NEW"};
...
void create_protocol_selector_panel(int x0, int y0);
...
//AFTER_RUN_FLASH_CALL       = refresh_objects;
  ...
  create_protocol_selector_panel( 250, 0);
...
void invalidate_data(void) {
  ...
  invalidate_ww(first, &w_protocol, 1, 0);
...
//int i;
  ...
  if (file) {
    ...
    fwrite(w_protocol.inp, 4, 1, file);
...
void create_protocol_selector_panel(int x0, int y0) {
  ...
  x = x0 + 5;
  y = y0 + 5;
  add_frame_box(x, y, w1, h1, "Protocol", 'S');
  y = y + h1;

  create_btn_hstring(
    1, x, y, w1, h2, protocols, 'M', &w_protocol, &protocol, &PROTOCOL_BTN);
}
...
//~--- Board Status
    ...
    if (go_on(err)) {
      ...
      exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+adc, &dig, NULL, &err);
      if (!err) {
        r->rea[0] = dig & 0x01;
...
//~--- Board Temperature
    ...
    if (go_on(err)) {
      ...
      pack16(&cmd, 0x01E0, adr);
      pack16(&cmd, 0x001F, cha);
      exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+adc, NULL, &ana, &err);
      if (!err) {
...
void digital_readout_callback(FL_OBJECT *obj, long data) {
  ...
  for (i=0; i<NL; i++) {
    ...
    if (go_on(err)) {
      ...
      pack16(&cmd, 0x01E0, adr);
      pack16(&cmd, 0x001F, i);
      exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+0, &dat, NULL, &err);
      if (!err) {
        r->rea[0] = dat;
        if (r->unk) r->was[0] = r->rea[0];
        r->sta = status_OK;
...
void analog_readout_callback(FL_OBJECT *obj, long data) {
  ...
  for (i=0; i<NL; i++) {
    ...
    if (go_on(err)) {
      ...
      pack16(&cmd, 0x01E0, adr);
      pack16(&cmd, 0x001F, i);
      exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+adc, NULL, &dat, &err);
      if (!err) {
        r->rea[0] = dat;
        if (r->unk) r->was[0] = r->rea[0];
        r->sta = status_OK;
...
void digital_commanding_callback(FL_OBJECT *obj, long data) {
  ...
  exec_PDS_cmd(cmd, w_protocol.inp[0]*0x80+0, NULL, NULL, &err);
