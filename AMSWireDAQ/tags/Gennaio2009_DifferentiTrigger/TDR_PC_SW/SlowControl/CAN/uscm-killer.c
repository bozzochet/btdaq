...
//~========
  ...
  for (i=0; i<4; i++) {
    if ((pchannel[i]->state_1 == START)) {
      if (P > 3) printf("JMDC%d GO\n", pchannel[i]->timer);
      status = new_CAN_SND_RCV(GO, pchannel[i]);
//    if ((i == 3)) delay(0.000810);
//    else          delay(Delay);
      delay(Delay);
    }
  }
...
//~--
  ...
  for (i=0; i<4; i++) {
    ...
    pRequest[i]->NodeAddress  = USCM_ID;
...
//~--
  ...
  printf("Usage: %s [<USCM_ID> [<ECHO length> [nnn [P]]]]\n",argv[0]);
...
//  else         nnn = 10;
    if (!(ccc % nnn)) {
      printf("Delay = %f: ccc = %d, eee[0] = %d, eee[1] = %d, eee[2] = %d, eee[3] = %d\n",
              Delay, ccc, eee[0], eee[1], eee[2], eee[3]);
      if (Auto) {
        ccc = 0;
        eee[0] = eee[1] = eee[2] = eee[3] = 0;
