...
void TrLadCal::Fill(CaloutDSP* cc) {
  ...
  for (int ii=0;ii<640;ii++){
    ...
    _Sigma[ii]     = cc->sig[ii]/(cc->S_lowthres*1.);
    _Status[ii]    = cc->status[ii];
  }
  for (int ii=640;ii<1024;ii++){
    _Pedestal[ii]  = cc->ped[ii]/8.;
    _SigmaRaw[ii]  = cc->rsig[ii]/8./(cc->sigrawthres*1.);
    _Sigma[ii]     = cc->sig[ii]/(cc->K_lowthres*1.);
    _Status[ii]    = cc->status[ii];
  }
  
  dspver=cc->dspver;
  S_lowthres=cc->S_lowthres;
  S_highthres=cc->S_highthres;
...
TrLadCal::TrLadCal(const TrLadCal& orig):TObject(orig){
  ...
  dspver=orig.dspver;
  S_lowthres=orig.S_lowthres;
  S_highthres=orig.S_highthres;
...
int  TrLadCal::Cal2Lin(float* offset){
  ...
  offset[2]= S_lowthres;
  offset[3]= S_highthres;
  offset[4]= K_lowthres;
  offset[5]= K_highthres;
  offset[6]= sigrawthres;
  offset[7]= (float) calstatus;
...
int  TrLadCal::Lin2Cal(float* offset){
  ...
  dspver      = (int)offset[1];
  S_lowthres    = offset[2];
  S_highthres   = offset[3];
  K_lowthres    = offset[4];
  K_highthres   = offset[5];
  sigrawthres = offset[6];
  calstatus   = (int)offset[7];
