...
//---------------------------------------------------------------------------
int DaqClass::Calibrate(int run,int lad,float timeout){

  char command[255];

  sprintf(command,"CALIBRATE %d %d",lad,run);
  return SendCommandWithReply(command,"READY","BUSY","Calibrate",timeout);
  
}

//---------------------------------------------------------------------------
int DaqClass::StartDaq(int run,GonClass* gon,float timeout){

  char command[255];
  //  Gon->GetPos();
  if(gon)
    sprintf(command,"START %d %f %f %f %",run,gon->ReadAlpha(),gon->ReadCraddle(),gon->ReadLin1(),gon->ReadLin2());
  else 
