...
//-------------------------------------------------
		if (argv<3) {
			ret=1;
			printf("Too few arguments: %s <run #>		<JINF #>		[type of file](everything for pdf, \"ps\" for PostScript)        \n                   [name prefix](default is \"\")			[dir](default is \"./Calibrations\")\n",argc[0]);
		}
		else if (argv==3) {
			RefMask(CPars,atoi(argc[1]), atoi(argc[2]), "");
			ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), atoi(argc[2]), "pdf", "");
		}
		if (argv>3) {
			char type[255];
			sprintf(type,"pdf");
			if (!strcmp(argc[3],"ps")) sprintf(type,"ps");
			if (argv==4) {
				RefMask(CPars,atoi(argc[1]),atoi(argc[2]), "");
				ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), atoi(argc[2]), type, "");
			}
			else if (argv==5) {
				RefMask(CPars,atoi(argc[1]), atoi(argc[2]), argc[4]);
				ret=SummaryComplete(CPars->CALPATH, atoi(argc[1]), atoi(argc[2]), type, argc[4]);
			}
			else if (argv>5) {
...
//    LPRINTF("%d : %d\n", ii, CPars->refmask&(1<<ii)); //only for debug
	...
	printf("We have %d file of calibrations for run #%d an Jinf #%d in the directory \"%s\" with prefix name \"%s\"\n",tdrcount, run_number, jinfnum, dir, nameprefix);
