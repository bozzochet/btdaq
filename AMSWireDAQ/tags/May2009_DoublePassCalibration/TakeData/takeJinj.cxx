...
int Calibrate(AMSWcom* node, int secondstep) {
  ...
  //	TESTRXDONE(node);This control cause an error if the JLV1 isn't in calibration mode
  ...
  if (secondstep) {
    SLPRINTF("CALIBRATION %d with positive second step\n", runnum);
    SLPRINTF("CALIBRATION %d with negative second step\n", runnum+1);
	}
  else {
  ...
  if(secondstep){
    for (int ii=0;ii<NSLAVE;ii++) {
      if((JJ->CPars->refmask&(1<<ii)) && strstr(JJ->CPars->SlaveConfFile[ii],"JINF")) {
		...
		if(tt==0) node->Calibrate(address,0,0x8008);//the positive second step of calibration...
		if(tt==1) node->Calibrate(address,0,0x8010);//the negative second step of calibration...
		int control=0;
		int cnt=0;
		while(control==0){
		  cnt++;
		  usize=node->GetCalibration(address,0x1);//Changing parameter here has no issue...
		  if(tt==0) PRINTF("We took %d events for positive second step calibration\n", (node->Event[usize-3]));
		  if(tt==1) PRINTF("We took %d events for negative second step calibration\n", (node->Event[usize-3]));
		  if((node->Event[usize-2]) & 0x1) {//the word before the last word is the calibrations status...
		    if (!((node->Event[usize-2]) & 0xfc)) {
  ...
  ret=SaveCalibration(node, runnum+tt);
...
int StartRun(AMSWcom *node, int nevents, int fake) {
  ...
  while(ControlOn) {
    ...
    if(evtcnt%5000==0&&evtcnt!=0&&printevent==1)  {
      printevent=0;
   //    trig->TriggerOff();
//       PRINTF("I have stopped the trigger\n");
//       //    sleep(2);
//       PRINTF("Prev Ev: Sec  %d  usec:%d \n ",unixtime.tv_sec,unixtime.tv_usec);
//       gettimeofday(&unixtime, NULL);
//       PRINTF("Now: %d Sec  %d  usec:%d \n ",evtcnt,unixtime.tv_sec,unixtime.tv_usec);    
      
//       node->Command(0xffff,0x2,0);
//       // PRINTF("JINJ: %d\n",node->Event[0]);
      
//       node->Command(0x013f,0x2,0);
//       // PRINTF("JINF: %d\n",node->Event[0]);
      
//       node->Command(0x013f013f,0x2,0);
//       // PRINTF("TDR01: %d\n",node->Event[0]);
	    
//       node->Command(0x023f,0x2,0);
//       // PRINTF("JLV1: %d\n",node->Event[0]);
           usleep(50000);
        PrintAllEventNumber(evtcnt);
	
/*	nt->Fill(node->Event[1],node->Event[usize-1],
		 JJ->Slave[1]->jinfevents[0],JJ->Slave[1]->jinfevents[1],
		 JJ->Slave[2]->jinfevents[0],JJ->Slave[2]->jinfevents[1],
		 JJ->Slave[1]->tdrevents[0][0],JJ->Slave[1]->tdrevents[1][0]);*/
      // for(int kl=0;kl<5000;kl++){
// 	usize=0;
// 	//	usleep(5000);
...
// 	if(usize>0) {
// 	  gettimeofday(&unixtime, NULL);
// 	  PRINTF("Next Ev: %d Sec  %d  usec:%d \n ",kl,unixtime.tv_sec,unixtime.tv_usec);
	  
// 	  evtcnt++;
// 	  PRINTF("Status JINJ %hx\n",node->Event[usize-1]);
// 	}
//       }
//       PRINTF("\n");
      
//       node->Command(0xffff,2,0);
//       PRINTF("JINJ: %d\n",node->Event[0]);
      
//       node->Command(0x013f,2,0);
//       PRINTF("JINF: %d\n",node->Event[0]);
      
//       node->Command(0x013f013f,2,0);
//       PRINTF("TDR01: %d\n",node->Event[0]);
      
//       node->Command(0x023f,2,0);
//       PRINTF("JLV1: %d\n",node->Event[0]);
//       //	  PrintAllEventNumber(evtcnt);
//       //sleep(5);
//       trig->TriggerOn();
//       sleep(1);
//       PRINTF("I have restarted the trigger\n");
    }
    printevent=1;
    usize=0;
    //	usleep(500);
    ...
    if (usize>0) {
      int a=node->Event[1];
      int b=node->Event[a+2];
      int c=a+b+1+1+1+1;
      if (c!=usize) PRINTF("There's a big problem in reading %d %d %d usize:%d\n!", a,b,c, usize);
	...
	printf("Event size = %d\n", node->GetEventSize());
      */
      ...
      size_t sizescritta=0;
      sizescritta=fwrite(node->Event,size_u_short, usize, datafile);
      //      sizescritta=fwrite(node->Event,size_u_short*usize, 1, datafile);
      if (sizescritta!=usize) PRINTF("There's a big problem %d %d\n!", sizescritta, usize);
      fwrite(&DSPCRC16, sizeof(DSPCRC16), 1, datafile);
