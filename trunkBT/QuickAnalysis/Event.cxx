...
bool Event::FindTracksAndVertex(){

  ClearTrack();//tra le altre cose fa il clear di _TrS e _TrK
  // h1=new TH2F("", "Hough Transform;inclinazione;distanza",
  // 		    250, -1, 1,
  // 		    250, -3000, 3000);
  ...
  for (int index_cluster = 0; index_cluster < NClusTot; index_cluster++) {
    ...
    if (side==0) {   // ( index, ( X or Y, Z ) )
	  ...
	  && GetAlignPar(jinfnum,tdrnum,2) < 300 ) {
	v_hitsS.push_back(make_pair(index_cluster, make_pair(current_cluster->GetAlignedPosition(), GetAlignPar(jinfnum, tdrnum, 2))));
	//printf("hit n° %d, x = %f, z = %f\n",index_cluster, current_cluster->GetAlignedPosition(), GetAlignPar(jinfnum, tdrnum, 2));
      }
    }
    else {
      if (find(_v_ladderK_to_ignore.begin(), _v_ladderK_to_ignore.end(), item)==_v_ladderK_to_ignore.end()
	  && GetAlignPar(jinfnum,tdrnum,2) < 300) {
	v_hitsK.push_back(make_pair(index_cluster, make_pair(current_cluster->GetAlignedPosition(), GetAlignPar(jinfnum, tdrnum, 2))));
  ...
  //  printf("ora iniziano i problemi2\n");
  ...
  //std::vector<std::pair<int,std::pair<double,double>>> vert_rejectsS;
  //std::vector<std::pair<int,std::pair<double,double>>> vert_rejectsK;
  ...
  for(int it=0;it<n_tr_to_search;it++){
    ...
    //_TrK and _TrS are vectors that can store the track we have already found
    _TrK.push_back(make_track(v_hitsK));
    _TrS.push_back(make_track(v_hitsS));
  ...
  std::pair<double,double> vx=vertex(_TrK[0],_TrK[1]), vy=vertex(_TrS[0],_TrS[1]);
...
void Event::Track(std::vector<std::pair<int,std::pair<double,double>>> &hits,std::vector<std::pair<int,std::pair<double,double>>> &rejects){

  std::vector<std::pair<int,std::pair<double,double>>> _hits;
  ...
  for(int itdr=NTDRS-1;itdr>=0;itdr--){
    ...
    int s=hits.size();
    for(int j=0;j<s;j++){
      if ( hits[j].second.second<pos+1 && hits[j].second.second>pos-1 ){    //layer belonging condition
	//	printf("pos=%f\n",pos);
...
pair<double,double> Event::Hough( vector< pair< int, pair< double, double >>> &vec ){
  ...
  //  printf("lol4\n");
  int xmax,ymax;
  TH1D *hprojX=h1->Projection(0,"O");
  TH1D *hprojY=h1->Projection(1,"O");
  xmax=hprojX->GetMaximumBin();
  ymax=hprojY->GetMaximumBin();
  best_dir.first=hprojX->GetXaxis()->GetBinCenter(xmax);
  best_dir.second=hprojY->GetXaxis()->GetBinCenter(ymax);
  delete hprojX;
  delete hprojY;
  delete h1;
  //  printf("MAX = ( %f, %f )\n",best_dir.first,best_dir.second);
  return best_dir;
}
...
void track::update(){
  ...
  if(ss>1){
    prod_angle = atan2(hits[1].second.first - hits[0].second.first, hits[1].second.second - hits[0].second.second);
    exit_angle = atan2(hits[ss-1].second.first - hits[ss-2].second.first, hits[ss-1].second.second - hits[ss-2].second.second);
...
track make_track(std::vector<std::pair<int,std::pair<double,double>>> hits){
  ...
  // unsigned int ss=hits.size();
  // if(ss>1){
  //   tr.prod_angle = atan2(hits[1].second.first - hits[0].second.first, hits[1].second.second - hits[0].second.second);
  //   tr.exit_angle = atan2(hits[ss-1].second.first - hits[ss-2].second.first, hits[ss-1].second.second - hits[ss-2].second.second);
  //   tr.prod_dist = cos(tr.prod_angle) * hits[0].second.first-sin(tr.prod_angle) * hits[0].second.second;
  //   tr.exit_dist = cos(tr.exit_angle) * hits[ss-1].second.first - sin(tr.exit_angle) * hits[ss-1].second.second;
  // }
  return tr;
}
...
std::pair<double,double> vertex(track tr1, track tr2){
  ...
  vert.second=( tr2.prod_dist*cos(tr1.prod_angle) - tr1.prod_dist*sin(tr2.prod_angle) )/
