...
int main(int argc, char **argv) {
    ...
    opt->addUsage(
            Form("  --rawdata <path/to/dir/with/raw> ............ Directory with raw data (%s is the default)",
                 DirRaw));
    opt->addUsage(
            Form("  --caldata <path/to/dir/with/cal> ............ Directory with cal data (%s is the default)",
                 DirCal));
    opt->addUsage(Form(
            "  --rootdata <path/to/dir/for/root> ........... Directory where to put ROOT file (%s is the default)",
            DirRoot));
    ...
    opt->addUsage(" ");
    opt->addUsage(
            "                                                To choose the clusterization thresholds: FOR ALL THE "
    ...
    opt->addUsage(
            "                                                    FOR ALL THE LADDERS, IGNORING THE ONE DEFINED IN "
            "ladderconf.dat:");
    opt->addUsage(
        ...
        for (size_t hh = 0; hh < fConf.NTDRS; hh++) {
            for (int ss = 0; ss < 2; ss++) {
                ...
                if (entries >= 1) {
                    ...
                    //	  printf("%f %f\n", mean, rms);
                    (dd1->hsignal[fConf.NTDRS * jj + hh][ss])->GetXaxis()->SetRangeUser(mean - 5.0 * rms,
                                                                                        mean + 9.0 * rms);
                    canvas->Update();
                    canvas->Modified();
                    canvas->Update();
