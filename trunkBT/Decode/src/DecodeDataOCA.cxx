...
bool DecodeDataOCA::ProcessCalibration() {
    ...
    // FIXME: Should skip, or read+check, file header for the calibration file
...
#define PERCENTILE 0.02
    ...
    // FIXME: Some test calibrations contain too many events, stop at 10k and use the first half for ped and sigma raw, and the second half for sigma
    // MD: tipo che
            ...
            cals[iTdr].rsig[iCh] =
                    ...
                    std::sqrt(std::accumulate(beginItr, endItr, 0.0f,
                                              [&](float acc, float curr) {
                                                  return acc +
                                                         (curr - cals[iTdr].ped[iCh]) * (curr - cals[iTdr].ped[iCh]);
                                              }) /
                              //		  static_cast<float>(signals[iTdr][iCh].size()));
            ...
            for (unsigned int iCh = 0; iCh < NVAS * NCHAVA; ++iCh) {
                ...
                if (thisVA != lastVA) {
                    ...
                    for (unsigned int iVACh = 0; iVACh < NCHAVA; ++iVACh) {
                        double sig =
                                signals[iTdr][thisVA * NCHAVA + iVACh][iEv] - cals[iTdr].ped[thisVA * NCHAVA + iVACh];
...
int DecodeDataOCA::ReadOneEventFromFile(FILE *file, EventOCA *event) {
    ...
    for (unsigned int iBoard = 0; iBoard < m_numBoards; ++iBoard) {
        ...
        if (nFrames != numChannels)
            std::cerr << "WARNING: payload length doesn't match number of channels in Event class (" << nFrames
                      << " vs "
                      << numChannels << ")\n";
            ...
            for (unsigned int iVal = 0; iVal < data.size(); ++iVal) {
                ...
                if (event->RawSignal[iJinf][iTDR][iCh] > (m_adcUnits * 4095)) {
                    printf("event = %d, Jinf = %d, TDR = %d, Channel = %d) rawsignal = %d (%d)\n", event->Evtnum, iJinf,
                           iTDR, iCh, event->RawSignal[iJinf][iTDR][iCh], data[iVal]);
                    //	  sleep(1);
                }
