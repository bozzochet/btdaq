...
void OpenVladimirFile(TString filename, std::vector<TH1F*>& histos);
...
void plotta_cal(){
  ...
  /*
  const int namsl0rootfiles = 3;
  TString amsl0rootfiles[namsl0rootfiles] = {"LEFP01_noVA.bin", "LEFP03_2.bin", "LEFP03_3.bin"};
  */
  /*
  const int namsl0rootfiles = 1;
  TString amsl0rootfiles[namsl0rootfiles] = {"LEFP03_17.bin"};
  */
  const int namsl0rootfiles = 5;
  TString amsl0rootfiles[namsl0rootfiles] = {"LEFP03_18.bin", "LEFP03_19.bin", "LEFP03_20.bin", "LEFP03_21.bin", "LEFP03_22.bin"};
  ...
  if (amsl0) {
    for (int ii=0; ii<namsl0rootfiles; ii++) {
      std::vector<TH1F*> histos;
      OpenVladimirFile(amsl0rootfiles[ii], histos);
      char nameouttemp[255];
      sprintf(nameouttemp, "%s", nameout);
      Summary(histos, amsl0rootfiles[ii], nameouttemp, 1);
      if (amsl0rootfiles[ii].Contains("LEFP03_")) {
	for (int jj=0; jj<3; jj++) {//3 since there're 3 comparisons: pedestal, sigma and sigmawas
	  comparison[jj].push_back(histos[4+jj]);//4+jj since the first 4 plots are other stuff (pedestal, sigma, sigma raw and average signal)
	}
      }
...
//         abiProcessEnvelope2(process_block, (void *) 1, block, TRUE);
    ...
    // Check envelope for fine time
        ...
        uint32_t ft   = __bswap_32(*((uint32_t *) ptr8));
        ptr8 += 4;
...
void prova() {
  ...
  const int namsl0rootfiles = 1;
  TString amsl0rootfiles[namsl0rootfiles] = {"./Data/L0/BLOCKS/USBL0_PG_LEFV2BEAM1/0005/519"};
  
  char fname[256] = "";
  sprintf(fname, "%s", amsl0rootfiles[0].Data());
