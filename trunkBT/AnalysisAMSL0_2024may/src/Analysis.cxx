...
void CreateTH3(TObjArray &hist, std::vector<TString> &vecName, std::vector<TString> &vecTitle, 
TString name, TString title, int nxbin, int xmin, int xmax,int nybin, int ymin, int ymax, int nzbin, int zmin, int zmax);

void CreateTH2(TObjArray &hist, std::vector<TString> &vecName, std::vector<TString> &vecTitle, 
TString name, TString title, int nxbin, int xmin, int xmax,int nybin, int ymin, int ymax);

void CreateTH1(TObjArray &hist, std::vector<TString> &vecName, std::vector<TString> &vecTitle,
TString name, TString title, int nbin, int xmin, int xmax);
//new
...
std::vector<TString> name_sigOverNoise_vs_cog;
std::vector<TString> title_sigOverNoise_vs_cog;
std::vector<TString> name_TotSig;
std::vector<TString> title_TotSig;
...
template <class Event, class RH> void BookHistos(TObjArray *histos, Long64_t entries, int _maxtdr, TChain *chain) {
  ...
  CreateTH2(*histos, name_sigOverNoise_vs_cog, title_sigOverNoise_vs_cog, "siOverNoise_vs_cog", "S/N vs Cog (Max Cl); Cog; S/N/",1024, 0, 1024, 500, 0, 30);
  CreateTH1(*histos, name_occupancy, title_occupancy, "occupancy", "occupancy; cog; Entries", 1024, 0, 1024);
  CreateTH1(*histos, name_TotSig, title_TotSig, "TotSig", "TotSig (Max Cl); TotSig (ADC); Entries", 500,0,2000);
  // all events
...
template <class Event, class RH> void FillCleanHistos(TObjArray *histos, int NClusTot, Event *ev, int index_event, TFile *f) {
  ...
  std::vector<TH2*> lsigOverNoise_vs_cog;
  std::vector<TH1*> loccupancy;
  std::vector<TH1*> ltotSig;
  
  /*nt xLadder[7] = {1,14,13,8,2,4,12};
  int yLadder[7] = {9,10,3,16,0,17,15};*/
  
  for (int ii=0; ii<nLadder; ii++) {
  
  	lTotSig_vs_cog.push_back( (TH2 *)( histos->FindObject(name_TotSig_vs_cog[ii].Data())) );
  	loccupancy.push_back( (TH2 *)( histos->FindObject(name_occupancy[ii].Data())) );
  	lsigOverNoise_vs_cog.push_back( (TH2 *)( histos->FindObject(name_sigOverNoise_vs_cog[ii].Data())) );
	ltotSig.push_back( (TH1*)(histos->FindObject(name_TotSig[ii].Data())) );
  }
	  	...
	  	if (clmax->GetLength() >= 1) {
	  			int Ladd = clmax->laddNum();
	  		  	if (Ladd >= 0 && Ladd <18) {
	  		  		...
	  		  		lsigOverNoise_vs_cog[Ladd]->Fill(cog,clmax->GetTotNoise() );
	  		  		ltotSig[Ladd]->Fill(tot);
	  		   	}
		}
	}
...
void CreateTH3(TObjArray &hist, std::vector<TString> &vecName, std::vector<TString> &vecTitle,
TString name, TString title, int nxbin, int xmin, int xmax,int nybin, int ymin, int ymax, int nzbin,
	int zmin, int zmax) {
	for (int ii=0; ii<nLadder; ii++) {
