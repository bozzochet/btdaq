/*=============================================================================
 ...
 $History: CQuickUsb.cpp $
 
 *****************  Version 18  *****************
 User: Bob Jacobson Date: 1/13/06    Time: 2:39p
...
CQuickUsb::CQuickUsb(const char *devName){
   isOpen = false;
   tempOpen = false;
   m_devName = new char[strlen(devName)+1];
...
int CQuickUsb::SetDeviceName(char *newDevName) {
  // if the current object is already open then force a close
  if (isOpen){
     result = Close();
     if(result == 0){
       lastError = QUSB_COULD_NOT_CLOSE;
       cerr << "CQuickUsb::SetDeviceName - Could not close device\n";
       return FALSE;
     }
  }
  delete [] m_devName;
  m_devName = new char[strlen(newDevName)+1];
...
int CQuickUsb::GetStringDescriptor(unsigned char index, char *str, unsigned short maxlen) {      
   // Open the device if it isn't already open
   if (!isOpen){
      tempOpen = true;
      result = Open();
      if (result == FALSE){
         Close();
	      ...
	      cerr << "CQuickUsb::GetStringDescriptor - Temp open failed\n";
         return FALSE;
      }
   }

   // Get the string descriptor
   result = QuickUsbGetStringDescriptor(hDevice, index, str, maxlen);
   if (result == FALSE) {
      return FALSE;
   }
...
int CQuickUsb::ReadSetting(unsigned short address, unsigned short *value) {
   ...
   if (!isOpen) {
      ...
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::ReadSetting - Temp open failed\n";
         return FALSE;
      }
   }
...
int CQuickUsb::WriteSetting(unsigned short address, unsigned short value) {
   ...
   if (!isOpen){
      ...
      if (result == FALSE) {
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteSetting - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::StartFpgaConfiguration(void) {
   ...
   if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::StartFpgaConfiguration - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteFpgaData(unsigned char *fpgadata, unsigned long datalength) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::WriteFpgaData - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::IsFpgaConfigured(unsigned short *isConfigured) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::IsFpgaConfigured - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteCommand(unsigned short address, unsigned char *data, unsigned short length) {
   if(!isOpen){
     ...
     if(result == FALSE){
       Close();
       lastError = QUSB_TEMP_OPEN_FAILED;
       cerr << "CQuickUsb::WriteCommand - Temp open failed\n";
       return FALSE;
     }
   }
...
int CQuickUsb::ReadCommand(unsigned short address, unsigned char *data, unsigned short *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::ReadCommand - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteData(unsigned char *data, unsigned long length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteData - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadData(unsigned char *data, unsigned long *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::ReadData - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadPortDir(unsigned short address, unsigned char *data) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::ReadPortDir - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WritePortDir(unsigned short address, unsigned char data) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WritePortDir - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadPort(unsigned short address, unsigned char *data, unsigned short *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::ReadPort - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WritePort(unsigned short address, unsigned char *data, unsigned short length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::WritePort - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::SetRS232BaudRate(unsigned long baudRate) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::SetRS232BaudRate - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadRS232(unsigned char portNum, unsigned char *data, unsigned short *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::ReadRS232 - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteRS232(unsigned char portNum, unsigned char *data, unsigned short length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteRS232 - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::FlushRS232(unsigned char portNum){
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::FlushRS232 - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::GetNumRS232(unsigned char portNum, unsigned short *length){
   if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::GetNumRS232 - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadI2C(unsigned short address, unsigned char *data, unsigned short *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::ReadI2C - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteI2C(unsigned short address, unsigned char *data, unsigned short length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteI2C - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::ReadSpi(unsigned char portNum, unsigned char *data, unsigned short *length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::ReadSpi - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteSpi(unsigned char portNum, unsigned char *data, unsigned short length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
        cerr << "CQuickUsb::WriteSpi - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::WriteReadSpi(unsigned char portNum, unsigned  char *data, unsigned short length) {
    if(!isOpen){
      ...
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteReadSpi - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::SetTimeout(unsigned long timeOut) {
    if(!isOpen){
      tempOpen = true;
      result = Open();
      if(result == FALSE){
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::SetTimeout - Temp open failed\n";
        return FALSE;
      }
    }
...
int CQuickUsb::GetDriverVersion(unsigned short * MajorDriverVersion, unsigned short * MinorDriverVersion, unsigned short * BuildDriverVersion) {
   result = QuickUsbGetDriverVersion(MajorDriverVersion, MinorDriverVersion, BuildDriverVersion);
   return result;
}

int CQuickUsb::GetDllVersion(unsigned short * MajorDllVersion, unsigned short * MinorDllVersion, unsigned short * BuildDllVersion) {
   result = QuickUsbGetDllVersion(MajorDllVersion, MinorDllVersion, BuildDllVersion);
   return result;
}

int CQuickUsb::GetFirmwareVersion(unsigned short * MajorFirmwareVersion, unsigned short * MinorFirmwareVersion, unsigned short * BuildFirmwareVersion) {
   // Open the device if it isn't already open
   if (!isOpen) {
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::GetFirmwareVersion - Temp open failed\n";
         return FALSE;
      }
   }

   result = QuickUsbGetFirmwareVersion(hDevice, MajorFirmwareVersion, MinorFirmwareVersion, BuildFirmwareVersion);
...
int CQuickUsb::WriteDataAsync(unsigned char *data, unsigned long length, unsigned char *transaction) {
   // Open the device if it isn't already open
   if (!isOpen) {
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::WriteDataAsync - Temp open failed\n";
         return FALSE;
      }
   }

   result = QuickUsbWriteDataAsync(hDevice, data, length, transaction);
...
int CQuickUsb::ReadDataAsync(unsigned char *data, unsigned long *length, unsigned char *transaction) {
   // Open the device if it isn't already open
   if (!isOpen) {
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::ReadDataAsync - Temp open failed\n";
         return FALSE;
      }
   }

   result = QuickUsbReadDataAsync(hDevice, data, length, transaction);
...
int CQuickUsb::AsyncWait(unsigned long *bytecount, unsigned char transaction, unsigned char immediate) {
   // Open the device if it isn't already open
   if (!isOpen) {
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::AsyncWait - Temp open failed\n";
         return FALSE;
      }
   }

   result = QuickUsbAsyncWait(hDevice, bytecount, transaction, immediate);
...
/*
int CQuickUsb::ReadDefault(unsigned short address, unsigned short *value) {
   // Open the device if it isn't already open
   if (!isOpen) {
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
         Close();
         lastError = QUSB_TEMP_OPEN_FAILED;
	      cerr << "CQuickUsb::ReadDefault - Temp open failed\n";
         return FALSE;
      }
   }

   result = QuickUsbReadDefault(hDevice, address, value);
...
signed long CQuickUsb::WriteDefault(unsigned short address, unsigned short value) {
   // Open the device if it isn't already open
   if (!isOpen){
      tempOpen = true;
      result = Open();
      if (result == FALSE) {
        Close();
        lastError = QUSB_TEMP_OPEN_FAILED;
	     cerr << "CQuickUsb::WriteDefault - Temp open failed\n";
        return FALSE;
      }
    }

   result = QuickUsbWriteDefault(hDevice, (QWORD) address,(QWORD) value);
...
int CQuickUsb::GetNumModules(char *nameList){
   if(nameList == NULL){
      return 0;
   }
   char * str = nameList;
...
int CQuickUsb::GetModuleName( char * nameList, unsigned char index, char *&name ) {
   ...
   numOfItems = GetNumModules(nameList);
