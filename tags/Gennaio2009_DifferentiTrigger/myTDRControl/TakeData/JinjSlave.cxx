...
ConfPars::ConfPars() {
  ...
  refmask=0;
	...
	memset(DeltaTdoubTrig,0,sizeof(DeltaTdoubTrig));
}
  ...
  fscanf(file,"%s  %s",&dummy,&CALPATH);
	...
	if (strstr(conf_file,"JLV1")){
		...
		for (int tdr=0; tdr<NTDRS; tdr++) {
			...
			DeltaTdoubTrig[tdr]=(short int)g*1000/40;
//			printf("SLowTrash: %hx, SHighTrash: %hx, KLowTrash: %hx, KHighTrash: %hx, SigmaRowTrash: %hx, DeltaTdoubTrig: %hx\n",
