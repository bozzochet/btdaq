...
while (1) {  
  ...
  for (aport=0; aport<1; aport++) {
    for (ouscm=1; ouscm<2; ouscm++) {
      for (oport=0; oport<1; oport++) {
        for (iuscm=1; iuscm<2; iuscm++) {
          for (iport=0; iport<1; iport++) {
            ...
            for (j=0; j<n_try; j++) {
              ...
              if (n_rcv != 33) {
                ...
                if (n_rcv == 1) {
                  printf("Received byte = %02X", d_rcv[0]);
                  if (isprint(d_rcv[0])) printf(" = %c", d_rcv[0]);
                  printf(".");
                }
                Read_serial_port(iport, 8192, &n_rcv, d_rcv, &err);
                if (err) PANIC("Read_serial_port failed\n");
