...
//  - send "syncronized" ECHO command from 4 JMDCs to up to 2 USCMs.
//  - manages 2 delays
//
//  A.Lebedev Oct 2005...
//
...
int JMDC_ID = 0x03;
int USCM1_ID = 0x152;    // 1xx stands for Newborn ID
...
//~--
  ...
  for (i=0; i<4; i++) {
    ...
    pRequest[i]->NodeAddress  = (i == 0 ? USCM1_ID : USCM2_ID);
...
//~--
  ...
  printf("Usage: %s [USCM1_ID [USCM2_ID [<ECHO length> [nnn [P]]]]]\n",argv[0]);

  USCM1_ID = 0x152;
  if (argc > 1) USCM1_ID = strtoul(argv[1], NULL, 16);
  ...
  nnn = 100;
  if (argc > 4) nnn = atoi(argv[4]);

  P = 0;
  if (argc > 5) P = atoi(argv[5]);
  
  printf("USCM1 ID = 0x%02X\n", USCM1_ID);
