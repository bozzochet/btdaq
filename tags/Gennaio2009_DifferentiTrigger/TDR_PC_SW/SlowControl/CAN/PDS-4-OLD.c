// file PDS-4-OLD.c
//
// PDS-4-OLD Controller - uses old CGS protocol
//
// A.Lebedev Dec-2007...
...
char *title = {"PDS-4-OLD  Controller"};              // global variable
...
//~--- Board Status
    ...
    if (go_on(err)) {
      ...
      exec_PDS_cmd(cmd, adc, &dig, NULL, &err);
      if (!err) {
        r->rea[0] = dig & 0x01;
...
//~--- Board Temperature
    ...
    if (go_on(err)) {
      ...
      pack16(&cmd, 0x01E0, adr);
      pack16(&cmd, 0x001F, cha);
      exec_PDS_cmd(cmd, adc, NULL, &ana, &err);
      if (!err) {
