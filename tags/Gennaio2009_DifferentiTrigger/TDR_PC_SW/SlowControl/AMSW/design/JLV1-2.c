...
char *title = {"JLV1-2:  Controller"};              // global variable
...
void write_config_file_callback(FL_OBJECT* obj, long data) {
  ...
  if (fl_get_button_numb(obj) == FL_RIGHT_MOUSE) {
    printf("The format below is preferred by AL\n");
    n = 4;
    for (i=0; i<n_reg/n; i++) {
      for (j=0; j<n; j++) {
        int k = i + j * n_reg / n;
        printf("%02X %04X    ", k, w->inp[k]&0x0000FFFF);
      }
      printf("\n");
    }
  }
  else {
    printf("The format below was approved by AB and VK at 11:04 on 12-Nov-2008\n");
    n = 8;
    for (i=0; i<n_reg/n; i++) {
      for (j=0; j<n; j++) {
        int k = i * n + j;
