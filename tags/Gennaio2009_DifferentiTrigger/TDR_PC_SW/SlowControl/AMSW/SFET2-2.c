...
char *title = {"SFET2/SFEA2 - 2  Controller"};      // global variable
...
void write_trigger_delay_callback(FL_OBJECT* obj, long data);
void read_trigger_delay_callback(FL_OBJECT* obj, long data);
...
void create_main_form(void) {
  ...
  for (brd=0; brd<5; brd++) {
    create_sfet_sfea_control_panel( 0+145*brd, 45, brd);
  }
...
void invalidate_data(void) {
  ...
  for (crt=0; crt<9; crt++) {
    for (brd=0; brd<5; brd++) {
      ...
      invalidate_r(first, &r_del[crt][brd],     1);
      invalidate_ww(first, &ww_del[crt][brd], 1, 0x00000000);
...
bool read_dat_config_file(char *filename) {
  ...
  if (file != NULL) {
    for (crt=0; crt<9; crt++) {
      for (brd=0; brd<5; brd++) {
        fread(ww_del[crt][brd].inp, 4, 1, file);
        for (thr=0; thr<6; thr++) {
...
bool write_dat_config_file(char *filename) {
  ...
  if (file != NULL) {
    for (crt=0; crt<9; crt++) {
      for (brd=0; brd<5; brd++) {
        fwrite(ww_del[crt][brd].inp, 4, 1, file);
        for (thr=0; thr<6; thr++) {
...
void update_crate_selector_panel(void) {
  ...
  if (crate != old_crate) {
    ...
    for (brd=0; brd<5; brd++) {
      ...
      _r  *r = &r_del[crate][brd];
      _ww *w = &ww_del[crate][brd];
...
{//~--- Panel ---
  ...
  int w1 = 145, h1 = 395, h2 = 30;

  fl_add_box(FL_DOWN_BOX, x0, y0, w1, h1, " ");

  x = x0 + 5;
  y = y0 + 5;
  add_frame_box(x, y, w1-10, h2, brd_nam[brd], 'L');
  y = y + h2;
}

{//~--- Power Control ---

  int w1 = 45, w2 = 35, w3 = 35, w4 = 20, h1 = 30, h2 = 15;
  ...
  obj = add_normal_button(x, y+h2, w2, h2, "OFF", 'N');
...
{//~--- Thresholds ---
  ...
  int x1, w;
  int w1 = 20, w2 = 30, w3 = 35, w4 = 30, w5 = 20, h1 = 15;
  char **txt1;
  char *txt1a[2] = {"0\n1\n2", "3\n4"};
  char *txt1b[2] = {"0\n1",    "2\n3"};
  ...
  x = x + w1;
  w = w2 + w3;
  add_frame_box(x, y, w,  h1, "Thr, mV", 'S');
  x = x + w;
  add_frame_box(x, y, w4, h1, "Vref",    'S');
  x = x + w4;
  ...
  for (thr=0; thr<6; thr++) {
    ...
    x = x + w3;
    create_btn_vstring(
      1, x, y,    w4, h1, i_e,  'N', &ww_thr[crate][brd][thr], &ref_msk, &THR_REF_BTN[brd][thr]);
    create_dat_box_vstring(
      1, x, y+h1, w4, h1,       'N',  &r_thr[crate][brd][thr], &ref_msk, &THR_REF_BOX[brd][thr]);
    x = x + w4;
    if (brd == 4 && thr != 0 && thr != 3) {
      ...
      add_frame_box(x1, y, w3+w4+w5, 2*h1, "N/A", 'S');
    }
    else {
      obj = add_normal_button(x, y,    w5, h1, "W", 'N');
        fl_set_object_callback(obj, write_threshold_callback, 10*brd+thr);
      obj = add_normal_button(x, y+h1, w5, h1, "R", 'N');
...
{//~--- Trigger Delay ---
  ...
  int w1 = 70, w2 = 45, w3 = 20, h1 = 15;

  x = x0 + 5;
  add_frame_box(x, y, w1, 2*h1, "Trigger\nDelay, us", 'S');
  x = x + w1;

  create_btn_vstring(
    1, x, y,    w2, h1, NULL,  'N', &ww_del[crate][brd], &del_msk, &DEL_BTN[brd]);
  add_hidden_btn_zones(DEL_BTN[brd], 3, 1, 16, 256);
  create_dat_box_vstring(
    1, x, y+h1, w2, h1,        'N', &r_del[crate][brd],  &del_msk, &DEL_BOX[brd]);
  x = x + w2;

  obj = add_normal_button(x, y,    w3, h1, "W", 'N');
    fl_set_object_callback(obj, write_trigger_delay_callback, brd);
  obj = add_normal_button(x, y+h1, w3, h1, "R", 'N');
    fl_set_object_callback(obj, read_trigger_delay_callback, brd);
  y = y + 2 * h1 + 5;
}
...
{//~--- Temperature ---
  ...
  x = x0 + 5;
  add_frame_box(x, y, w1, h1, "Temperature", 'S');
  x = x + w1;
...
{//~--- Actel ID (Version) ---
  ...
  create_dat_box_vstring(
    1, x, y, w2, h1, 'N', &r_ver[crate][brd], &ver_msk, &VER_BOX[brd]);
  x = x + w2;
...
void write_trigger_delay_callback(FL_OBJECT* obj, long data) {
  ...
  exec_SDR2_SC_command(WRITE, adr, del_adr, w->inp[0], NULL, NULL, &err);
...
void read_trigger_delay_callback(FL_OBJECT* obj, long data) {
  ...
  r->was[0] = r->rea[0];
  exec_SDR2_SC_command(READ, adr, del_adr, 0, &rea, NULL, &err);
...
void read_all_sfet_sfea_callback(FL_OBJECT* obj, long data) {
  ...
  for (i=0; i<5; i++) {
    ...
    if (go_on(err)) read_trigger_delay_callback(NULL, i);
...
//~---
    ...
    show_btn_string(1, NULL, &BTN);
    cod = BTN->i;
    sprintf(b, "%5.2f", 20.0 / 1000.0 * (float)cod);
    fl_set_object_label(BTN->obj, b);

    BOX = DEL_BOX[brd];
    cod = r_del[crate][brd].rea[0] & 0x03FF;
    sprintf(b, "%5.2f", 20.0 / 1000.0 * (float)cod);
    show_dat_box_string(1, NULL, b, &BOX);

//~---

    show_dat_box_string(1, NULL, "%+5.1f", &TMP_BOX[brd]);
