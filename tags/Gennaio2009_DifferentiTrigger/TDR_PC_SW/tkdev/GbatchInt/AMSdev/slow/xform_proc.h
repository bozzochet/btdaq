//  $Id: xform_proc.h,v 1.1.1.1 2007/11/13 09:56:12 zuccon Exp $
...
int Y_line, Y_current, ind=0, i_log[14], ind_l=-1;
time_t ti;

typedef char ch80[80];
typedef char ch256[256];

ch80 names[50][7], host_name[7];
...
Forms log_f[14];
FL_OBJECT *quit, *help, *but, *b_open_log[7], *b_open_elog[7], *heade;
Obje check_[10][8], numb_[20][8], spare[8], ind_[2], stop_h[14], stati_[9];
...
int Y_c, Y_step=22, i;
  ...
  fl_set_object_bw(obj_buf,6);
  fl_set_object_color(obj_buf,FL_TOP_BCOL,FL_TOP_BCOL);
  stati_[8] = add_stat(8,X+10,Y+20,320,Y_step,"host");
  Y_c=Y+20+Y_step;
  for (i=0; i<7; i++) {
    stati_[i] = add_stat(i,X+10,Y_c,320,Y_step,host_name[i]);
    Y_c += Y_step;
  }
  stati_[7] = add_stat(7,X+10,Y_c,320,Y_step,"sum");
...
int Xf_init(void) {
 ...
 /*HeaD OF form */
  ...
  Y_current += 170;
  add_comp_box(0,2,5,Y_current,340,170,host_name[2]);
  add_comp_box(0,3,350,Y_current,340,170,host_name[3]);
  Y_current += 170;
  add_comp_box(0,4,5,Y_current,340,170,host_name[4]);
  add_comp_box(0,5,350,Y_current,340,170,host_name[5]);
  Y_current += 170;
  add_comp_box(0,6,5,Y_current,340,170,host_name[6]);
  add_stat_box(350,Y_current,340,220,"statistics");
  Y_current +=170;
  check_[1][7]= add_check_field(1,7,15,Y_current+5,320,40,"DB disk used");
