...
void AMSEvent::_retkevent(){
  ...
  printf("Building Clusters ...");fflush(stdout);
  if(retr>=0){
    printf("done\n");fflush(stdout);
    printf("Building Hits ...");fflush(stdout);
    int retr2=TrRecon::gethead()->BuildTrRecHits();
    if(retr2>=0)
      printf("done\n");fflush(stdout);
      printf("Building tracks ...");fflush(stdout);
      int retr3=TrRecon::gethead()->BuildTrTracks();
      if(retr3)printf("done\n");else printf("failed \n");fflush(stdout);
  }
}
