...
//--- Calocube ---
c_calocube *calocube=NULL;
char *calocube_conf="CALOCUBE.conf";
...
int StartUp(AMSWcom* node){
	...
	//eugenio: Tutta l'inizializzazione di calocube è inserita quando la classe è creata
	//		 Per questo JinjSlave->Init() chiamato da Jinj->InitJinfs() non è stato sovrascritto...
	//		 Il guadagno di tempo che si avrebbe nello spostare conf_qusb e read_info in Init() mi sembra davvero minimo
...
int StartRun(AMSWcom *node, int nevents, int fake) {
	...
	calocube = NULL;
	calocube = new c_calocube(calocube_conf);

	if(!calocube->is_enabled()) {
		delete calocube;
		calocube =NULL;
	} else {
		if(daq!=1 || WritingMode!=0) {
	...
	if(calocube) {
		...
		while(!ready) {
			...
			if(elapsed>calocube->init_time_out) {
				PRINTF("Initial Trigger Check: Calocube has not received any trigger\n");
				return 1;
			}
			...
			if(ready) {
				...
				if (usize>0) {
					PRINTF("Initial Trigger Check: AMS has received a trigger as well\n");
					return 1;
				}

				//This is necessary to manually enable AMS trigger
				//Question how much time after TriggerOn AMS trigger is really On?
				cout << "Meglio dormire, va" << endl;
				sleep(10);
				cout << "Che fatica vivere..." << endl;
	...
	while(ControlOn) {
		...
		/*if(calocube) {
		bool buttavia;
		calocube->check_data_ready(buttavia);
		printf("CALOCUBE is READY? %d\n", (buttavia? 1 : 0));
		}*/
		...
		if (usize>0) {
			...
			if(daq){
				if (WritingMode==0){
					...
					if(is_calocube_enabled)
						if(calocube->take_data_with_ams(unixtime, (char *)&Head, sizeof(Head))!=0) {
							++errcnt;
							is_calocube_enabled=false;
						}
		...
		if (calocube && !is_calocube_enabled) StopRun(0);//if Calocube is in the DAQ but we had an error regarding Calocube data taking, we force exit in this way
	}
