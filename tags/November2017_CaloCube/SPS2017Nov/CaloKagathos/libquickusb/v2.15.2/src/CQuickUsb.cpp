...
QRETURN CQuickUsb::ReadDataEx(PQBYTE data, PQULONG length, QULONG flags) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbReadDataEx(m_hDevice, data, length, flags) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
QRETURN CQuickUsb::WriteDataEx(PQBYTE data, PQULONG length, QULONG flags) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbWriteDataEx(m_hDevice, data, length, flags) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
QRETURN CQuickUsb::BulkAbort(PQBULKSTREAM bulkStream) {
    // Don't politely open as we must already be open for this call

    if (QuickUsbBulkAbort(m_hDevice, bulkStream) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
    ...
    // Don't politely close module because we're still performing async requests
    return true;
}
...
QRETURN CQuickUsb::ReadBulkDataStartStreamToFile(PCQCHAR path, QULONG numBuffers, QULONG bufferSize, QULONG maxTransfers, PQBYTE streamID, QULONG flags) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbReadBulkDataStartStreamToFile(m_hDevice, path, numBuffers, bufferSize, maxTransfers, streamID, flags) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
QRETURN CQuickUsb::WriteBulkDataStartStreamFromFile(PCQCHAR path, QULONG numBuffers, QULONG bufferSize, QULONG maxTransfers, PQBYTE streamID, QULONG flags) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbWriteBulkDataStartStreamFromFile(m_hDevice, path, numBuffers, bufferSize, maxTransfers, streamID, flags) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
QRETURN CQuickUsb::GetStreamStatus(QBYTE streamID, PQULONG status, PQULONG error) {
    // Don't politely open as we must already be open for this call

    if (QuickUsbGetStreamStatus(m_hDevice, streamID, status, error) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
// Statistics API //
////////////////////
QRETURN CQuickUsb::ResetStatistic(Stat statistic) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbResetStatistic(m_hDevice, statistic) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
...
QRETURN CQuickUsb::GetStatistic(Stat statistic, StatUnit unit, PQFLOAT value, QULONG flags) {
    if (!OpenPolitely()) {
        return false;
    }

    if (QuickUsbGetStatistic(m_hDevice, statistic, unit, value, flags) == 0) {
        QuickUsbGetLastError(&m_lastError);
        QuickUsbGetLastDriverError(&m_lastDriverError);
        ClosePolitely();
        return false;
