...
void init_CAN_LeCroy_bus(int8 bus, int16 *err) {
...
//  *lst_pnt++ = 0x0000;    // secondary header
    for (i=0; i<pRequest->DataCount; i+=2) {
      *lst_pnt++ = ((int16)pRequest->Data.p8[i] << 8) | pRequest->Data.p8[i+1];
    }
  }
...
//  if (P > 0) printf("PAD: Sending command...\n");
    ...
    if (!pReply->Error) {
      pReply->Data.p8 = RP_dat;
      if (RP_dat) {
        if (pReply->DataCount > RP_siz) pReply->Error = 0xDEAD;
        if (!pReply->Error) {
          memcpy(RP_dat, pReply->Contents, pReply->DataCount);
        }
      }
    }
  }
