...
void SlowGui::FillTbsPage(Int_t page) {
  ...
  for (Int_t grp=0; grp<2; grp++) {
    ...
    tTBSGetVOLT[page][grp]->Connect("Clicked()","SlowGui",this,Form("GetTbsADCBiasVoltage(=%d,1)",page));
  ...
  for (Int_t grp=0; grp<2; grp++) {
    for (Int_t tdr=0; tdr<NTdrTPSFE; tdr+=2) {
      ...
      aLabel = new TGLabel(vframe2,Form("TDR %d",TPSFEtoTDR(grp+2*page,tdr)));
      vframe2->AddFrame(aLabel,fL74);
      ...
      aLabel = new TGLabel(vframe2,Form("TDR %d",TPSFEtoTDR(grp+2*page,tdr+1)));
      vframe2->AddFrame(aLabel,fL74);
...
void SlowGui::UpdateTKBiasVoltage(Int_t Tbs, ushort group, int cold) {
  ...
  ushort adcvoltage=TbsVolt[Tbs][group][cold];
...
void SlowGui::UpdateTbsBiasVoltage(Int_t Tbs, ushort group, Int_t empty) {
  ...
  tTBSADChot[Tbs][group]->SetText((empty)?"---":(ADCmode)?Form("0x%04X",TbsVolt[Tbs][group][0]):Form("%3.1f",ADCtoVolt(TbsVolt[Tbs][group][0],group,0)));
  tTBSADCcold[Tbs][group]->SetText((empty)?"---":(ADCmode)?Form("0x%04X",TbsVolt[Tbs][group][1]):Form("%3.1f",ADCtoVolt(TbsVolt[Tbs][group][1],group,1)));
...
void SlowGui::UpdateTbsBiasCurrent(Int_t Tbs, ushort Tdr) {
  ...
  tTBSADCcurrent[Tbs][Tdr]->SetText((ADCmode)?Form("0x%04X",TbsCurrent[Tbs][Tdr]):Form("%3.1f",ADCtoCurrent(TbsCurrent[Tbs][Tdr],Tbs, Tdr,1 )));
...
void SlowGui::UpdateTbsBiasCurrents(Int_t Tbs, Int_t empty) {
  ...
  for (sTBSTDR tdr=kTDR0; tdr<=kTDR11; tdr++) {
    tTBSADCcurrent[Tbs][tdr]->SetText((empty)?"---":(ADCmode)?Form("0x%04X",TbsCurrent[Tbs][tdr]):Form("%3.1f",ADCtoCurrent(TbsCurrent[Tbs][tdr], Tbs, tdr,1)));
    gClient->NeedRedraw(tTBSADCcurrent[Tbs][tdr]);
    ...
    FillTKGraph(pwg, pwgpos, ADCtoCurrent(TbsCurrent[Tbs][tdr], Tbs, tdr,1));
