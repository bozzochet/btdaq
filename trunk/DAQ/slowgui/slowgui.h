...
class SlowGui {
 ...
 private:
  ...
  TGNumberEntry *tAMSWaddr, *tTimerDel;
  ...
  int AMSWmode, AMSWport;

  int TCrate;

 public:
  SlowGui(const TGWindow *p, UInt_t w, UInt_t h, int tcrate, unsigned int jinfaddr, int port);
  virtual ~SlowGui();
  ...
  void EnableS9053();
  void TestAMSW(unsigned int addr);
